...
public class RootProcess2 {
    ...
    private RootThread2 stThread;
    private RootThread2 errThread;

    public RootProcess2(String arg) {
        if (arg != null && arg.length() > 0) {
            init(arg);
        }
    }

    public static Object getObject(RootProcess2 arg) {
        return arg.locak;
    }
    ...
    private void init(String arg) {
        try {
            ...
            processBuilder = Runtime.getRuntime().exec(arg);
            synchronized (locak) {
                locak.wait(10);
            }
            data_out_put_stream = new DataOutputStream(processBuilder.getOutputStream());
            //准备线程读取流信息
            stThread = new RootThread2(this, "str", processBuilder.getInputStream(), baos_str);
            errThread = new RootThread2(this, "err", processBuilder.getErrorStream(), baos_err);

            synchronized (locak) {
                locak.wait(10);

            }
            RootThread2.setIsRunning(true);
            stThread.start();
            errThread.start();
            LogUtil.loge("process inited ok &  thread str err start !");
        } catch (IOException e) {
            e.printStackTrace();
            LogUtil.loge("init IOException "+e.getCause()+" ; "+e.getMessage().toString());
        } catch (InterruptedException e) {
            e.printStackTrace();
            LogUtil.loge("init InterruptedException "+e.getCause()+" ; "+e.getMessage().toString());
        }
    }
...

    {
        ...
        RootThread2.setIsRunning(false);
    }
...
public RetValue execute(ProcessHelper arg)
{
    return execut(arg);
}
...
//                    LogUtil.loge("status  "+result.toString());
                ...
                e.printStackTrace();
                LogUtil.loge(" IOException " + e.getCause() + " = " + e.getMessage().toString());
            } catch (InterruptedException e) {
                e.printStackTrace();
                LogUtil.loge("InterruptedException" + e.getCause() + " = " + e.getMessage().toString());
            } catch (TimeoutException e) {
                e.printStackTrace();
                LogUtil.loge("time exception " + e.getCause() + " = " + e.getMessage().toString());
            }
        }
        return result;
    }
